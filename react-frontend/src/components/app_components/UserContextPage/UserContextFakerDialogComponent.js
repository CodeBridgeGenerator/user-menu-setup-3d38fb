...
import userContextFakerFactory from "./userContextFakerFactory";
...
const UserContextFakerDialogComponent = (props) => {
...
client.service("users")
    ...
    const onRun = async () => {
        let fakeData = userContextFakerFactory(props.user,fakerCount,roleIds,positionIds,profileIds,userIds,companyIds,branchIds,departmentIds,sectionIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("userContext").create(elem));
...
export default connect(mapState, mapDispatch)(UserContextFakerDialogComponent);
