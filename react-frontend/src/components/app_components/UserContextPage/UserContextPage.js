...
import UserContextDatatable from "./UserContextDataTable";
...
import UserContextFakerDialogComponent from "./UserContextFakerDialogComponent";
import UserContextSeederDialogComponent from "./UserContextSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const UserContextPage = (props) => {
    ...
    const filename = "userContext.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("userContext");
    ...
    useEffect(() => {
        ...
        client
            .service("userContext")
            .find({ query: { $limit: 10000 , role : urlParams.singleUsersId ,position : urlParams.singleUsersId ,profile : urlParams.singleUsersId ,user : urlParams.singleUsersId ,company : urlParams.singleUsersId ,branch : urlParams.singleUsersId ,department : urlParams.singleUsersId ,section : urlParams.singleUsersId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("userContext").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "UserContext", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("userContext").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="userContext-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="userContext-datatable">
                    <UserContextDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <UserContextEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <UserContextCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <UserContextFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <UserContextSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(UserContextPage);
